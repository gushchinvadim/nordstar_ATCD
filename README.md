...
**Стек технологий:**
- **Backend:** Django 5.x + Python 3.14 + Django REST Framework
- **Database:** PostgreSQL 14+
- **Frontend:** React 18 + Vite + CSS Modules
- **PDF-генерация:** WeasyPrint
- **Excel-отчёты:** openpyxl, pandas

**Структура репозитория:**
```text
nordstar_ATCD/
├── backend_ATCD/         # Django-приложения
│   ├── core/             # Базовые утилиты, management commands
│   ├── people/           # Студенты, персонал (Staff)
│   ├── training/         # Программы, модули, этапы, разделы
│   ├── execution/        # Группы, зачисления, оценки, ComplianceLog, ИУП
│   ├── references/       # Справочники (ВС, аудитории, лицензии, локации)
│   ├── reporting/        # Отчёты РАУЦ и ФРДО
│   └── docs/             # Views и шаблоны для генерации PDF-документов
|   |__ landing/         # Главная страница 
├── frontend_ATCD/        # React-приложение (Vite)
── README.md
...
## 📄Генерация документов и Аудит (v2.0)
Все ключевые действия (ознакомление с расписанием, инструктаж,
подтверждение посещаемости, ознакомление с оценкой, получение документов) фиксируются в модели ComplianceLog.

### Основные документы группы

- Приказ о зачислении / об окончании (ОК) / об отчислении (ОТ)
- Расписание занятий и Журнал подготовки (посещаемость + тематический план)
- Задания на тренировку АСП (Суша/Вода)
- Сертификаты / Удостоверения

- Часовые пояса: Время подписи автоматически конвертируется из UTC в локальное время локации проведения занятия.
- Ролевая модель: В журнале четко разделяются подписи инструктора (mentor), методиста/куратора (curator) и студента.
...
2. Указать новые даты, инструкторов, аудитории
3. Система сгенерирует отдельное расписание и тематический план
ИУП сохраняются в БД и могут быть перегенерированы даже после архивации основной группы.

## 🗄 Структура базы данных (Ключевые модели)
### execution/
- Group: Группа обучения. Важно: mentor (Наставник/инструктор) и curator (Специалист 1 категории/методист, ответственный за документы) разделены.
- Enrollment: Зачисление студента в группу
- Assessment: Оценка по разделу
- ComplianceLog: Журнал аудита и цифровых подписей
* IndividualStudyPlan: Индивидуальный учебный план
## training/
- Course, Module, Stage, Section, Subsection (иерархия программы обучения)

## people/
- Student (слушатель), Staff (преподаватель/методист/руководитель)
